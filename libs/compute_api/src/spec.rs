...
pub enum PageserverProtocol {
    /// The original protocol based on libpq and COPY. Uses postgresql:// scheme, and also accepts
    /// postgres://. Default if no scheme is specified.
    #[default]
    Libpq,
...
impl PageserverProtocol {
    /// Parses the protocol from a connstring scheme. Errors if the connstring is an invalid URL.
    pub fn try_from_connstring(connstring: &str) -> anyhow::Result<Self> {
