...
fn acquire_lsn_lease_with_retry(
    ...
    loop {
        ...
        let result = try_acquire_lsn_lease(connstrings, auth, tenant_id, timeline_id, lsn);
        match result {
            Ok(Some(res)) => {
                return Ok(res);
            }
...
fn try_acquire_lsn_lease(
    ...
    let connstrings = connstrings.split(',').map(|s| s.to_string()).collect_vec();
    let shard_count = connstrings.len();
    let mut leases = Vec::new();

    for (shard_number, connstring) in connstrings.into_iter().enumerate() {
        ...
        let auth = auth.clone();
        let lease = match PageserverProtocol::try_from_connstring(&connstring)? {
...
fn acquire_lsn_lease_grpc(
    ...
    tokio::runtime::Handle::current().block_on(async move {
        ...
        let req = pageserver_page_api::LeaseLsnRequest { lsn };
        match client.lsn_lease(req).await {
