...
impl proto::PageService for GrpcPageServiceHandler {
    ...
    async fn lease_lsn(
        ...
        // Validate and convert the request, and decorate the span, and convert it.
        let req: page_api::LeaseLsnRequest = req.into_inner().try_into()?;

        span_record!(lsn=%req.lsn);

        // Attempt to acquire a lease.
        let expires = match timeline.renew_lsn_lease(req.lsn, timeline.get_lsn_lease_length(), &ctx)
        {
